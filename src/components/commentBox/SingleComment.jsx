...
function SingleComment() {
    ...
    const { id, title, description, author, name, Email, professional, proTitle, date, duration, views, link, thumbnail, tags, like, comments } = blog || {}
    const { user, } = useSelector(state => state.user)
    const { email, token } = user
    const [authOpp, setAuthOpp] = useState(false);  //for comment edit & delete
    ...
    // edit & delete with checking auth
    useEffect(() => {
        if (isChecked) {
            if (email === Email) {
                setAuthOpp(true)
    ...
    const deleteCommentHandler = (index) => {
        if (isChecked) {
            if (email === Email) {
                ...
                const newComments = comments.filter(com => com.id !== index);
                const deletedObject = {
                    id,
                    title,
                    description,
            ...
            <div className=" snap-center overflow-hidden rounded-lg  shadow-md m-5">
                {editable ? (
                    <form onSubmit={editSubmitHandler}>
                        <input type="text" value={input} onChange={(e) => setInput(e.target.value)} className="w-screen h-auto" />
                    </form>
                ) : (
                    <table className="w-full h-auto bg-white text-left text-sm text-gray-500">
                        ...
                        {comments?.map((s) => (
                            <tbody key={s.i} className="divide-y divide-gray-100  rounded-xl ">
                                <hr />
                                    ...
                                    <td className="px-6 py-4">
                                        ...
                                        {authOpp ? (
                                            <div className="flex justify-end gap-2 md:gap-4">
                                                <div onClick={() => deleteCommentHandler(s.id)} className="cursor-pointer" x-data="{ tooltip: 'Delete' }">
                                                    <svg
                                                ...
                                                <div onClick={() => commentEditHandler(s.id)} className="cursor-pointer" x-data="{ tooltip: 'Edite' }" >
                                                    <svg
                                                        xmlns="http://www.w3.org/2000/svg"
                                                        fill="none"
                                                        viewBox="0 0 24 24"
